...
static void draw_top(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
    ...
    lv_draw_rect(&layer, &rect_black_dsc, &(lv_area_t){0, 0, CANVAS_SIZE - 1, CANVAS_SIZE - 1});
#endif
...
#else
    lv_draw_rect(&layer, &rect_white_dsc, &(lv_area_t){0, 21, 68 - 1, 42 - 1});
    lv_draw_rect(&layer, &rect_black_dsc, &(lv_area_t){1, 22, 66 - 1, 40 - 1});
#endif
